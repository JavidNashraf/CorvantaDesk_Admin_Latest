          ...
          <div className="flex gap-[16px]">
            ...
            <div className="flex flex-col gap-[4px]">
              ...
              <p className="text-xs text-[#474761]">
                {createdOn
                  ? moment(createdOn)?.format(localStorage.getItem('dateFormat'))
                  : null}
              </p>
            </div>
          </div>
...
export const FeedbackDetails = () => {
  ...
  useEffect(() => {
    console.log("fd", articlesFeedback);
  }, [articlesFeedback]);
  ...
  return (
    <>
      <Spin spinning={feedbackLoading || ticketLoading || departmentsLoading}>
        ...
        <div className="bg-[#000000] m-[40px] p-[32px] rounded-[8px]">
            ...
            onSubmit={async (values) => {
              if (values?.assignTo) {
                const final = {
                  ...
                  clientId: articlesFeedback.createdBy.toString(),
                };
        ...
        <div className="bg-[#000000] m-[40px] p-[32px] rounded-[8px]">
          <h6 className="text-white text-[20px] mb-[10px]">Feedback Details</h6>
          <div className='text-[#474761]' dangerouslySetInnerHTML={{ __html: articlesFeedback?.description }} />
        </div>
        <div className="m-[40px] p-[32px] bg-[#000000] rounded-[8px]">
          <div className="flex gap-[12px]">
            {articlesFeedback?.articleImage && !imgError ? (
              ...
              <div className="w-[90px] text-[#474761] rounded-[8px] object-cover border-1 border-blue-600 flex items-center justify-center text-[16px] font-medium">
                No Image
              </div>
            )}
            <div className="flex flex-col gap-[8px] ">
              <p className="text-[24px] text-[#FFFFFF] ">
                {articlesFeedback?.article?.title || 'No Title'}
              </p>
              <p className="text-[#474761] text-[14px]">
                Created On{' '}
                {articlesFeedback?.createdOn
                  ? moment(articlesFeedback?.createdOn).format(localStorage.getItem('dateFormat'))
                  : 'N/A'}{' '}
                by{' '}
                {clients?.find(
          ...
          <div className="flex justify-between gap-[40px] mt-[40px]">
            {articlesFeedback?.article ? (
              <p
                className="text-base text-[#FFFFFF]"
                dangerouslySetInnerHTML={{
                  __html: articlesFeedback?.article.bodyText,
                }}
              />
            ) : (
              <p className="text-base text-[#474761]">
               {"No Description"}
              </p>
            )}
          </div>
