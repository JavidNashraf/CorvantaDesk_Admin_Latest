...
import { supportTicketduration } from 'store';
...
export const SupportTicketDuration = () => {
  ...
  const { ticketDuration, loading } = useSelector((state) => state?.reports);
  useEffect(() => {
    if (ticketDuration && Object.keys(ticketDuration)?.length > 0) {
      for (let data in ticketDuration) {
        let obj = ticketDuration[data]?.find((items) => {
