...
import { suppotTicketReplycount } from 'store';
...
export const SupportTicketReplyCount = () => {
  ...
  useEffect(() => {
    if (replyReports && Object.keys(replyReports)?.length > 0) {
      for (let data in replyReports) {
        let obj = replyReports[data]?.find((items) => {
          const { filterBy } = items
          return filter?.toLowerCase() === filterBy?.toLowerCase()
        })
        const details = obj?.supportReplyDetails || []
        let resultObj = {}
        for (let detail of details) {
          if (detail?.[filter] && !(xAxis.some((value) => value == detail[filter]))) {
            xAxis.push(detail[filter])
          }
          resultObj[`${detail[filter]}`] = detail?.["count"]
        }
        filterObj[data] = {
          ...resultObj,
        }
